function [X,A_tilde,K] = generate_GK_nd(Z,n,sigma_kernel)
% Generate n-D embeded data at higher-dimensional space
% Input:
% Z: Data points of variation sources (p-dimensional) (rows are data
% points)
% n: Dimension of higher-dimensional space where data Z are embeded
% Output:
% X: Data points of Z embeded in higher-dimensional space (rows are data points)
...
display('K eigenvalues');
% [V1,D1,U1] = svd(K,'econ');
% diag(D1)
% 
% %Try to see the column space of A
% display('SVD on A')
% [V,D,U] = svd(A,'econ');
% diag(D)
